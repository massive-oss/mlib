...
class FileTest
{
	...
	public function TestGetRecursiveDirectoryListing():Void
	{
		...
		var sub = dir1.resolvePath("sub");
		var sub_xyz = dir1.resolvePath("sub/sub.xyz");
		files = dir1.getRecursiveDirectoryListing(null, false, true);

		Assert.isTrue(indexOfPath(files, sub) < indexOfPath(files, sub_xyz));
	...
	static function indexOfPath(files: Array<File>, match: File): Int {
		var matchPath = match.nativePath;
		for (i in 0...files.length) {
			if (files[i].nativePath == matchPath) return i;
		}
		throw 'No file match found';
	}
